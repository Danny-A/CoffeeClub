...
import { revalidateBean, revalidateRoaster } from '@/lib/utils/revalidation';
...
export async function createBeanAction(input: CreateBeanInput) {
  const id = uuidv4();
  ...
  await revalidateBean(response.data.insertIntobeansCollection.records[0].id);
  await revalidateRoaster(input.roasterId);
